...
/*-------------------*/
    ...
    function enovathemes_addons_header_html($header_id, $header_type){
        ...
        if (!is_wp_error($headers) && array_key_exists($header_id,$headers)) {
            ...
            echo '<header id="et-'.$header_type.'-'.esc_attr($header_id).'" class="'.esc_attr(implode(" ", $class)).'">'.do_shortcode(gzuncompress($headers[$header_id]['content'])).'</header>';
        } else {
            ...
            if ($headers_query->have_posts()){
                ...
                while($headers_query->have_posts()) { $headers_query->the_post();
                    ...
                    if ($content) {
                        ...
                        $header_css = $upload_dir['basedir'].'/elementor/css/post-'.$header_id.'.css';
                        if (file_exists($header_css)) {
                            $styles = file_get_contents($header_css);
                        }
    ...
    function enovathemes_addons_footer_html($footer_id){
        ...
        if ($iframe == 'true' || $footer_async == "false" || $disable_async == "true") {
            ...
            if (!is_wp_error($footers)) {
                echo do_shortcode(gzuncompress($footers[$footer_id]['content']));
            } else {
            ...
            if ($footers_query->have_posts()){
                ...
                while($footers_query->have_posts()) { $footers_query->the_post();
                    ...
                    if ($content) {
                        ...
                        $footer_css = $upload_dir['basedir'].'/elementor/css/post-'.$footer_id.'.css';
                        if (file_exists($footer_css)) {
                            $styles = file_get_contents($footer_css);
                        }
        ...
        if ( false === ( $banners = get_transient( 'enovathemes-banners' ) ) ) {
            ...
            if ($banner->have_posts()){
                ...
                while($banner->have_posts()) { $banner->the_post();
                    ...
                    $content   = (is_plugin_active( 'elementor/elementor.php' )) ? $pluginElementor->frontend->get_builder_content($banner_id,false) : get_the_content();
                    
                    $styles = '';

                    $banner_css = $upload_dir['basedir'].'/elementor/css/post-'.$banner_id.'.css';
                    if (file_exists($banner_css)) {
                        $styles = file_get_contents($banner_css);
                    }
        ...
        if ( false === ( $megamenu = get_transient( 'enovathemes-megamenu' ) ) ) {
            ...
            if ($megam->have_posts()){
                ...
                while($megam->have_posts()) { $megam->the_post();
                    ...
                    $content = (is_plugin_active( 'elementor/elementor.php' )) ? $pluginElementor->frontend->get_builder_content($megam_id,false) : get_the_content();

                    $megamenu_html .= '<div id="megamenu-'. $megam_id . '" '.implode(' ', $megamenu_data).'>';
                        $megamenu_html .= $content;
                    $megamenu_html .= '</div>';

                    $styles = '';

                    $mm_css = $upload_dir['basedir'].'/elementor/css/post-'.$megam_id.'.css';
                    if (file_exists($mm_css)) {
                        $styles = file_get_contents($mm_css);
                    }

                    $megamenu[$megam_id] = array(
                        'id' => $megam_id,
                        'title' => get_the_title($megam_id),
                        'content' => gzcompress($megamenu_html),
                        'data' =>$megamenu_data,
                        'styles' =>$styles
                    );
                }
                    ...
                    if ($vehicle_params != false) {
                        ...
                        foreach ($vehicle_params as $param) {
                            $cmb_term_vehicle->add_field( array(
                                'name'    => esc_html__( ucfirst($param), 'enovathemes-addons' ),
               ...
               function enovathemes_addons_show_vehicles_meta_info_in_columns( $string, $columns, $term_id ) {
                ...
                function enovathemes_addons_add_new_vehicles_columns( $columns ) {
                    ...
                    if ($vehicle_params != false) {
                        ...
                        foreach ($vehicle_params as $param) {
                            $columns['vehicle_'.$param] = esc_html__( ucfirst($param),'enovathemes-addons' );
                        }
            ...
            ---------------------*/
                                    ...
                                    <tbody>
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <small><?php esc_html_e('If number of records are over 1000 - 1500 rows, separate CSV into smaller size files with up to 1000 - 1500 rows per file, or increase memory limit','enovathemes-addons'); ?></small>
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
                ...
                function enovathemes_addons_map_csv(){
                    ...
                    if(isset($_POST['map']) && !empty($_POST['map']) && isset($_POST['csv-file']) && !empty($_POST['csv-file']))
                    {
                        ...
                        if (($handle = fopen($_POST['csv-file'], "r")) !== FALSE) {
                            ...
                            foreach ($csv as $index => $row) {
                                if ($index > 0) {
                                    ...
                                    foreach ($row as $index => $record) {
                                        $vehicle_data[$headers[$index]] = $record;
                                    }

                                    if (!empty($vehicle_data)) {
                                        array_push($data,$vehicle_data);
                                    }
                                }
                            }
                                ...
                                if (!term_exists($vehicle, 'vehicles')) {
                                    ...
                                    if ( ! is_wp_error( $term ) )
                                    {
                                        ...
                                        if ($term_id) {
                                            ...
                                            foreach ($vehicle_meta as $key => $value) {
                            ...
                            $output['done'] = $done;
                            $output['skip'] = $skip;
                            $output['taxonomy-link'] = '<a class="button button-primary button-next" href="'.admin_url( 'edit-tags.php?taxonomy=vehicles&post_type=product', is_ssl() ? 'https' : 'http' ).'">'.esc_html__("View vehicles","enovathemes-addons").'</a>';
                ...
                function et_render_vehicles_table($post,$terms = true,$checkbox = true){
                    ...
                    if (isset($post) && !empty($post)) {
                        ...
                        if ($vehicle_params != false) {
                            ...
                            foreach ($vehicle_params as $param) {
                                $thead .= '<th>'.esc_html__(ucfirst($param),'enovathemes-addons').'</th>';
                            }
                ...
                function enovathemes_addons_render_vehicle_filter_attribute($attribute,$first){
                    ...
                    if ($first) {
                        ...
                        if (!is_wp_error($vehicles_first_param) && !empty($vehicles_first_param)){
                            ...
                            $data[] = 'data-label="'.esc_attr(esc_html__( $attribute['label'], 'enovathemes-addons' )).'"';
                            
                            $output = '<div class="vf-item '.$attribute['attr'].'" '.implode(' ', $data).'>';
                                $output .= '<select name="'.$attribute['attr'].'" disabled>';
                                    $output .= '<option class="default" value="">'.esc_html__( $attribute['label'], 'enovathemes-addons' ).'</option>';
                    ...
                    if (isset($_POST['vehicle']) && !empty($_POST['vehicle'])) {
                        ...
                        if (!empty($return)) {
                            ...
                            if (!empty($vehicle_data) && isset($vehicle_data['error'])) {
                                ...
                                $output['error'] = esc_html__("Duplicate record");
                            }
                ...
                function enovathemes_before_shop_loop_universal_products_title() {
                    if (isset($_POST['universal']) && $_POST['universal'] == 1 && !isset($_GET['ajax'])) {
                        ...
                        if (!is_wp_error($universal_products)) {
                            ...
                            echo '<h5 class="universal-title woocommerce-no-products-found">'.esc_html__("No products found matching the filter criteria. But here are some universal products:").'</h5>';
                        }
...
--------------------*/
    ...
    function enovathemes_addons_dashboard(){
        ...
        $sticky_dashboard_disable = (isset($modes['sticky_dashboard']) && $modes['sticky_dashboard'] == 1) ? true : false;

        if($sticky_dashboard_disable == false){

            $dashboard_list = array();

            if (class_exists('Woocommerce')) {
                $dashboard_list['account'] = esc_html__("Dashboard","enovathemes-addons");
                $my_account_link      = (class_exists('Woocommerce')) ? get_permalink(get_option('woocommerce_myaccount_page_id')) : home_url('/');
            }

            $dashboard_list['categories'] = esc_html__("Categories","enovathemes-addons");
            $dashboard_list['compare'] = esc_html__("Compare","enovathemes-addons");
            $dashboard_list['vfilter-toggle'] = esc_html__("Car filter","enovathemes-addons");
            $dashboard_list['product-search'] = esc_html__("Search","enovathemes-addons");
            ...
            if ($wishlist == "false") {unset($dashboard_list['wishlist']);}
            if ($compare == "false") {unset($dashboard_list['compare']);}

            if (is_active_sidebar('shop-widgets') && empty($shop_sidebar) && !defined('ENOVATHEMES_ADDONS')) {
                $shop_sidebar = 'true';
            }

            if (is_active_sidebar('blog-widgets') && empty($blog_sidebar) && !defined('ENOVATHEMES_ADDONS')) {
                $blog_sidebar = 'true';
            }
            if (
                (is_singular('post') && !empty($post_sidebar)) ||
                ((is_home() || is_category() || is_tag()) && !empty($blog_sidebar))
            ) {
                $dashboard_list['post-sidebar'] = esc_html__("Sidebar","enovathemes-addons");
            } elseif(class_exists('Woocommerce') && ((is_shop() || is_tax('product_cat') || is_tax('product_tag')) && !empty($shop_sidebar))) {
                $dashboard_list['shop-filter'] = esc_html__("Filter","enovathemes-addons");
                unset($dashboard_list['product-search']);
            }

            $dashboard_list['arrow-top'] = esc_html__("Top","enovathemes-addons");

            echo '<ul class="sticky-dashboard">';
            foreach ($dashboard_list as $item => $label) {

                $href = $class = $item_class = '';

                switch ($item) {
                    case 'account':
                        $href = (is_user_logged_in()) ? '#' : $my_account_link;
                    break;
                    case 'categories':
                        $href = '#';
                    break;
                    case 'wishlist':
                        $href = $my_account_link.'wishlist';
                    break;
                    case 'compare':
                        $href  = '#';
                    ...
                    case 'post-sidebar':
                        $href  = '#';
                        $class = 'post-sidebar content-sidebar-toggle blog';
                    break;
                    case 'arrow-top':
                        $href  = '#wrap';
                        $class = 'to-top';
                ...
                echo '<li class="'.esc_attr($item).$item_class.'" ><a class="'.esc_attr($class).'" href="'.esc_url($href).'" title="'.esc_attr($label).'">';

                    if('compare' == $item){
                        echo '<span class="compare-contents"></span>';
                    } elseif('wishlist' == $item){
                        echo '<span class="wishlist-contents"></span>';
                    }
...
/*-------------------*/
    ...
    function enovathemes_addons_breadcrumbs() {
        ...
        $blog_text     = (!empty(get_option('page_for_posts'))) ? get_the_title( get_option('page_for_posts') ) : get_bloginfo();
            ...
            -------------------*/
                ...
                if ( is_category() ) {
                    ...
                    if ($this_cat->parent != 0) {
                        ...
                        $output .= $text_before . single_cat_title('', false) . $text_after;
                    } else {
                        $output .= $text_before . single_cat_title('', false) . $text_after;
                    }
                ...
                if (is_tag()) {
                    ...
                    $output .= '<a href="' . $home_link . '">' . $home_text . '</a>'.$link_after;
                    $output .= '<a href="' . $blog_link . '">' . $blog_text . '</a>'.$link_after;
                    $output .= $text_before . single_tag_title('', false) . $text_after;
...
/*-------------------*/
    ...
    function search_product() {
        ...
        if (empty($keyword)) {
            echo wp_json_encode(['products' => [], 'tags' => [], 'attributes' => []]);
            wp_die();
        }

        $keyword_parts = explode(' ', $keyword);
        $transient_key = 'search_' . md5(serialize($keyword_parts));
        ...
        $args = [
            ...
            'post_status'            => 'publish',
            'fields'                 => 'ids',
            'search_prod_title'      => $keyword_parts,
            'search_prod_content'    => true,
        ];

        $products_by_title = new WP_Query($args);
        ...
        $products_by_sku = new WP_Query($sku_args);

        // Merge unique IDs
        $product_ids = array_unique(array_merge(
            $products_by_title->posts,
            $products_by_sku->posts
        ));

        // Get Products (use wc_get_products for performance)
        ...
        foreach ($products as $product) {
            ...
            $products_array[] = [
                ...
                'sku'       => esc_html__('SKU',"enovathemes-addons").': '.$product->get_sku(),
        ...
        set_transient($transient_key, $result_json, WEEK_IN_SECONDS);
        ...
        function enovathemes_addons_vin_decoder($vin,$data_only = false){
            ...
            if ($vin && $vin_decoder && $vin_key && $vehicle_params) {
                ...
                if ( $return && ! empty( $return ) ) {
                    ...
                    return array('error' => esc_html__("VIN decoding error. Make sure the vin is valid, and try again, otherwise, contact site administrator","enovathemes-addons"));
                }
...
/*----------------*/
    ...
    function et_posts_ajax_query($atts="",$query_options=""){
        ...
        if (isset($atts['unique_id']) && !empty($atts['unique_id'])) {
            $unique .= '_'.$atts['unique_id'];
        } else {
            foreach ($atts as $key => $value) {
                if (!empty($value)) {
                    $unique .= '_'.$key.'_'.$value;
                }
            } 
        }

        $elementor_editor = (\Elementor\Plugin::$instance->editor->is_edit_mode() || \Elementor\Plugin::$instance->preview->is_preview_mode()) ? true : false;

        if ( $elementor_editor || false === ( $post_query = get_transient( $unique ) ) ) {
            ...
            if($posts->have_posts()){
                ...
                // do not set an empty transient - should help catch private or empty accounts.
                if ( $elementor_editor == false && ! empty( $output ) ) {
...
/*----------------*/
    ...
    function woo_products_ajax_query($atts="",$query_options=""){
        ...
        if ($elementor_editor || (isset($atts['nocache']) && $atts['nocache'] == true) || false === ( $product_query = get_transient( $unique ) ) ) {
            ...
            if (!is_wp_error($megamenu)) {
                ...
                foreach ($megamenues as $mega_menu){
                    if (isset($megamenu[$mega_menu])) {
                        $data[$mega_menu] = do_shortcode(gzuncompress($megamenu[$mega_menu]['content']));
                    }
                }
                if (!empty($data)) {
                    wp_send_json($data);
