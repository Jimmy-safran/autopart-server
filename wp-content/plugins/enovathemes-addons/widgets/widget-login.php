	...
	class  Enovathemes_Addons_WP_Widget_Login extends WP_Widget {
		...
		public function widget( $args, $instance ) {
	 	...
	 	public function form( $instance ) {
			...
			$defaults = array(
	 		...
	 		foreach ($links as $label => $link) {
	 			$defaults[$label.'_label'] = esc_html__(ucwords(str_replace('_',' ',$label)));
	 		}
			...
			<?php
				...
				foreach ($links as $label => $link) { ?>
					...
					<p>
						<label for="<?php echo $this->get_field_id($label.'_label'); ?>"><?php echo esc_html__(ucwords(str_replace('_',' ',$label)).' label',"enovathemes-addons"); ?></label>
						<input class="widefat" id="<?php echo $this->get_field_id($label.'_label'); ?>" name="<?php echo $this->get_field_name($label.'_label'); ?>" type="text" value="<?php echo $instance[$label.'_label']; ?>" />
					</p>
		...
		public function update( $new_instance, $old_instance ) {
			...
			foreach ($links as $label => $link) {
				...
				if(!isset($instance[$label.'_label'])){
					$instance[$label.'_label'] = esc_html__(ucwords(str_replace('_',' ',$label)).' label',"enovathemes-addons");
				}

				if(!isset($new_instance[$label.'_label'])){
					$new_instance[$label.'_label'] = esc_html__(ucwords(str_replace('_',' ',$label)).' label',"enovathemes-addons");
				}

				$instance[$label.'_label']  = strip_tags( $new_instance[$label.'_label'] );
