...
function enovathemes_addons_include_dynamic_styles() {
	...
	if (is_plugin_active( 'elementor/elementor.php' )) {
	    ...
	    ---------------*/
	        ...
	        if (!is_wp_error($headers)) {
                ...
                if ($mobile_header != "none" && $mobile_header != "default" && array_key_exists($mobile_header,$headers)) {
        			wp_enqueue_style('mobile-header-styles', plugins_url('/css/mobile-header-styles.css', dirname(__FILE__) ));
                	wp_add_inline_style( 'mobile-header-styles', enovathemes_addons_minify_css($headers[$mobile_header]['styles']));
                }

                if ($desktop_header != "none" && $desktop_header != "default" && array_key_exists($desktop_header,$headers)) {
                	wp_enqueue_style('desktop-header-styles', plugins_url('/css/desktop-header-styles.css', dirname(__FILE__) ));
                	wp_add_inline_style( 'desktop-header-styles', enovathemes_addons_minify_css($headers[$desktop_header]['styles']));
                }
	    ...
	    ---------------*/
	        ...
	        if (!is_wp_error($footers)) {
                ...
                if ($footer != "none" && $footer != "default" && array_key_exists($footer,$footers)) {
        			wp_enqueue_style('footer-styles', plugins_url('/css/footer-styles.css', dirname(__FILE__) ));
                	wp_add_inline_style( 'footer-styles', enovathemes_addons_minify_css($footers[$footer]['styles']));
                }
	...
	if ( ! empty( $dynamic_css ) ) {
        ...
        if (is_file($file)) {
        	...
        	wp_enqueue_style('dynamic-styles', plugins_url('/css/dynamic-styles.css', dirname(__FILE__) ),'',rand(1,10).'.'.rand(1,10).'.'.rand(1,10));
        }
