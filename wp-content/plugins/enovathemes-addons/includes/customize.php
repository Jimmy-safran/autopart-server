...
/*-------------------*/
    ...
    $woo_attributes = et_get_attribute_taxonomies();
    if (!empty($woo_attributes) && !is_wp_error($woo_attributes)) {
        foreach( $woo_attributes as $attribute) {
            $woo_attributes_opt[$attribute->attribute_name] = $attribute->attribute_name;
        }
    }
...
/*-------------------*/
    ...
    new \Kirki\Field\Checkbox_Switch(
        [
            ...
            'label'       => esc_html__( 'Disable default sticky dashboard?', 'enovathemes-addons' ),
            'section'     => 'misc_section',
            'default'     => false,
        ]
    );
