...
/*-------------------*/
    ...
    function mobex_enovathemes_post_nav($post_type,$post_id){
        ...
        <nav class="post-single-navigation <?php echo esc_attr($post_type) ?> mob-hide-false">  
          <?php if(!empty($next_post)) {echo '<a rel="prev" href="' . esc_url(get_permalink($next_post->ID)) . '" title="'.esc_attr__("Previous ","mobex").$post_type.'">'.$post_prev_text.' '.$post_type.'</a>'; } ?>
          <?php if(!empty($prev_post)) {echo '<a rel="next" href="' . esc_url(get_permalink($prev_post->ID)) . '" title="'.esc_attr__("Next ","mobex").$post_type.'">'.$post_next_text.' '.$post_type.'</a>'; } ?>
        </nav>
        <?php 
    }
...
/*-------------------*/
    ...
    function mobex_enovathemes_wishlist_compare_quickview($layout,$product){
        ...
        if ($quickview == "true" && $layout != 'comp') {
            $output.='<div title="'.esc_html__("Quick view","mobex").'" class="en-quick-view"></div>';
        }
    ...
    function mobex_enovathemes_loop_product_title($layout) { ?>
        ...
        $output.='<div class="post-body">';
            $output.='<div class="post-body-inner">';
                ...
                if ($layout == 'grid' || $layout == 'list' || is_woo_pcc()){
