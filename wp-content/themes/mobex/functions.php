...
---------------*/
    ...
    function mobex_enovathemes_register_required_plugins() {
        ...
        if (class_exists('Woocommerce')) {
            $plugins[] = array(
                'name'      => esc_html__('Variation Swatches for WooCommerce', 'mobex'),
                'slug'      => 'variation-swatches-for-woocommerce',
            );
            $plugins[] = array(
                'name'      => esc_html__('Currency Switcher for WooCommerce', 'mobex'),
        ...
        ---------------*/
            ...
            function mobex_enovathemes_wishlist_toggle_single(){
                ...
                if($wishlist == "true"){
                    ...
                    echo '<a class="wishlist-toggle '.esc_attr($class).'" data-product="'.esc_attr($product->get_id()).'" href="'.esc_url(get_permalink(get_option('woocommerce_myaccount_page_id'))).'wishlist'.'" title="'.$title.'"></a><span class="wishlist-title">'.$title.' '.$wishlist_count.'</span>';
                }
                if($compare == "true"){
            ...
            <?php }
                ...
                add_action( 'woocommerce_before_add_to_cart_form', 'mobex_enovathemes_summary_banner', 1 );
        ...
        ---------------*/
            ...
            function mobex_enovathemes_quick_view($id) {
                ...
                <?php if (isset($_POST["id"]) && !empty($_POST["id"])): ?>
                    ...
                    <div class="qwc layout1"><div class="quick-view-wrapper loaded woocommerce"><div class="quick-view-wrapper-close"></div>
                        <?php
...
---------------*/
    ...
    function mobex_enovathemes_scripts() {
        ...
        if (!is_admin()) {
            ...
            wp_enqueue_script( 'controller', MOBEX_ENOVATHEMES_TEMPPATH . '/js/controller.js', array('jquery'), wp_get_theme()->get('Version'), true);
